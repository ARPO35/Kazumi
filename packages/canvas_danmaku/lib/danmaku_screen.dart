...
class _DanmakuScreenState<T> extends State<DanmakuScreen<T>>
  ...
  int _time = 0;
  int _frameSkip = 0;
  /// 弹幕渲染目标帧率 (Hz)，Linux 上软件渲染时降低以减轻 CPU 负担
  static const int _targetFps = 30;

  void _tick(Duration elapsed) {
    _frameSkip++;
    final tickValue = elapsed.inMilliseconds + _lastTick;
    // 仅每 N 帧更新一次 UI，降低渲染频率 (60Hz → 30fps)
    if (_frameSkip >= (60 ~/ _targetFps)) {
      _frameSkip = 0;
      _notifier.value = tickValue;
    }
    if (_time++ > 10) {
      _time = 0;
      _lazyTick(tickValue);
